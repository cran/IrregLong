...
This dataset contains some rows corresponding to times at which phenobarbital given and others at which a blood draw was taken to determine serum phenobarbital concentration. For the visit intensity model, the event of interest is having a concentration measured, so we set the variable event=1 if concentration was measured and zero otherwise. It is plausible that the dose of phenobarbital at the last administration, as well as the concentration at the last measurement, are informative about the time of the next concentration measurement, and so we create lagged versions of these variables. Ordinarily, this can be done within the iiwgee function. However, since in this dataset dose is recorded only when a new dose is given, and some infants had multiple concentration measurements without a new dose, the lagged dose variable needs to be created before running iiwgee. The dose variable is carried forward until a new dose is observed, through multiple calls to lagfn. The data is then restricted to those time points when a concentration measurement was taken. The original pharmacokinetic analysis restricts attention to the first 16 days of life, which we also do here. Finally, the IrregLong package requires the variable identifying subjects to be numeric rather than a factor, so we create a new numeric id variable. 
...
Since Apgar score is not significant, we consider whether the model can be simplified - first by including Apgar score as an indicator ($\geq 5$ vs. $<5$), then by removing variables that are not significant. We then consider whether interaction terms are needed.
```{r}
...
i$m
```
It thus appears that last dose per kilo birthweight, last concentration, and the interactions between last dose per kilo birthweight and concentration are predictive of the time to the next concentration measurement.
